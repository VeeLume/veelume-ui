<script lang="ts">
	 ...
	 * repositions). Deliberately a plain panel like `Surface.FilterButton`,
	 * not a bits-ui Popover — the overlay wave upgrades both together.
	 *
	...
	const position = $derived(klass || 'top-full right-0 mt-2');
...
{#if open}
	<!-- Transparent click-catcher, same dismissal discipline as FilterButton. -->
	<button
		type="button"
	...
	<div
		...
		class="absolute z-50 flex max-h-96 w-80 flex-col overflow-hidden rounded-lg border
		       border-border bg-popover text-popover-foreground shadow-lg {position}"
	>
					...
					<li class="border-b border-border last:border-b-0" class:bg-accent-tint={!n.read}>
						...
						{:else}
							<div class="flex items-start gap-2 px-3 py-2.5">
								<span
									class="w-4 shrink-0 text-center text-sm font-bold {notifLevelClass[n.level]
										.glyph}"
